...
if (isset($_GET['api'])) {
    ...
    // ─── Validate ID: only safe alphanumeric/underscore/hyphen ──
    function validateId($id) {
        return is_string($id) && preg_match('/^[a-z0-9_-]+$/i', $id);
    }

    // ─── Enforce request body size limit (10 MB) ─────────────
    function checkRequestSize($limitBytes = 10485760) {
    ...
    function rebuildImageArchive() {
        // Hold the archive lock for the entire read+build+write so a
        // concurrent save cannot interleave a partial JSON state.
        $archiveLock = PATH_IMG_ARCHIVE . '.lock';
        $archiveFh   = acquireLock($archiveLock);
...
MD;
        ...
        $written = file_put_contents($tmpFile, $md);
        if ($written !== false) {
        ...
        releaseLock($archiveFh, $archiveLock);
    }
    ...
    if ($action === 'save_mcu' && $_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        if (!validateId($newId) || ($oldId !== null && !validateId($oldId))) {
        ...
        if (!empty($input['image_b64'])) {
            ...
            if ($imgData === false || strlen($imgData) > 8388608) {
                http_response_code(400);
                echo json_encode(['ok' => false, 'error' => 'Image invalid or exceeds 8 MB']);
                exit;
            }
            if (getimagesizefromstring($imgData) === false) {
                http_response_code(400);
                echo json_encode(['ok' => false, 'error' => 'Image data is not a valid image']);
                exit;
            }
            $ext = 'jpg';
    ...
    if ($action === 'sse') {
        ...
        while ((time() - $startTime) < $maxRuntime) {
            ...
            if (file_exists($versionFile)) {
                $vLockFile = $versionFile . '.lock';
                $vfh = fopen($vLockFile, 'c');
                if ($vfh && flock($vfh, LOCK_SH)) {
                    $raw = file_get_contents($versionFile);
                    flock($vfh, LOCK_UN);
                    fclose($vfh);
                    @unlink($vLockFile);
                } else {
                    if ($vfh) { fclose($vfh); @unlink($vLockFile); }
                    $raw = file_get_contents($versionFile); // fallback
                }
